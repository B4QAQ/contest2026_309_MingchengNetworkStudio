...
| `hd_rk3506_rpmsgtest.c` 阻塞读卡死                                        | ✅**已修复 (v8)** | 主循环阻塞 read() 且超时判定在 read 之后 → M0 不回包时 read 永久阻塞，10s 超时永不触发（v7 板上验证卡死即此）。v8 改 poll(fd,POLLIN,剩余 ms)+read（CLOCK_MONOTONIC 计时），超时 FAIL 退出并打印诊断：`ls /dev \| grep rpmsg` 有 `/dev/rpmsg-rpmsg-mcu0-test`（NS 公告创建）= M0 完成链路握手（问题在 A7 收发），缺失 = M0 卡 wait_for_link_up（未收 A7 mailbox3 kick）或未启动。端点改名 rpmsg-mcu0-test→**rpmsg-mcu0-echo**（避免与 M0 NS announce 同名节点冲突；路由靠 dst 0x4003）。A7/M0 协议链路已静态核对一致：link_id 0x03、ept 0x4003、A7 set_status→notify→mailbox3 kick {CMD=0x03,DATA=RMSG} 释放 M0 link_state（首次 kick→env_isr(vq6=remote tvq)→tx_callback）；A7 收包 isr 以 RPTUN_NOTIFY_ALL 上报，rproc_virtio_notified(RSC_NOTIFY_ID_ANY) 两 vring 全处理，不依赖 rsc notifyid 0/1。M0 实际行为待 v8 板上验证 |
