...
| U 盘 vfat 配置缺口                                                        | ✅**已修复 (v8)** | v7 defconfig 无 CONFIG_FS_FAT：USB MSC 枚举出 /dev/sda 没问题，但 `mount -t vfat` 因 vfat 类型未注册而失败。v8 加 `CONFIG_FS_FAT=y`+`FAT_LCNAMES`+`FAT_LFN`。验收：插 USB-A 口 → ls /dev 见 sda → `mount -t vfat /dev/sda /tmp/usb`（/tmp/usb rcS 预建）→ ls /tmp/usb |
| `hd_rk3506_rpmsgtest.c` 阻塞读卡死                                        | ⚠️**A7 侧全修, M0 卡 link-up 前待定位 (v8d 探针已加)** | 主循环阻塞 read() 且超时判定在 read 之后 → M0 不回包时 read 永久阻塞。v8 改 poll(剩余 ms)+read；v8b 修复 rpmsg 共享窗 cached→uncached（M0 无 cache 无一致性，vring 元数据失同步根因）。**v8c 板上确认 M0 已启动**（日志宏修复后 "M0 firmware booted at 0xfff84000 (27120 bytes)" 出现，SIP_MCU_CFG 成功），但探针仍 used=0 = M0 未消费未发送。**v8d 源码级核对完成**：rk3506 mailbox 是 V2 语义（dts compatible rk3576-mailbox：单通道、bit0=消息到达、bit8=trigger mode、写需 HIWORD WE）；SDK M0 HAL MBOX_ChanEnable 写 A2B_INTEN=WE\|1 是对的（此前"裸写无 WE"怀疑不成立）；kick 格式 A7{CMD=0x03,DATA=magic} 与内核/M0 回调三方一致；A7 master set_status(DRIVER_OK)→notify→mbox3 kick 确认发出。剩余未知=M0 是否活到 rpmsg init / kick 是否被 M0 中断消费。**v8d rpmsgtest 超时探针新增 mbox 寄存器转储**（MBOX3/MBOX0 A2B INTEN + STATUS、B2A STATUS）：INTEN bit0=1→M0 活到 rpmsg init；只有 bit8→M0 卡 rpmsg 前（需接 M0 串口 UART4 GPIO1_C2/C3 @1500000）；STATUS bit0=1→kick 无人收（M0 INTMUX/NVIC 路径断）；bit0=0→M0 ISR 已消费；B2A bit0=1→M0 反向 kick 过我们。端点 rpmsg-mcu0-echo（路由靠 dst 0x4003）。A7/M0 协议链路静态核对一致：link_id 0x03、ept 0x4003、A7 收包 isr RPTUN_NOTIFY_ALL→两 vring 全处理 |
| U 盘 MSC bulk 传输挂死                                                    | ⚠️**双层根因修复 (v8c 完成机制 + v8d FIFO 能力)，待板上验收** | v8b INFO 跟踪定位三层完成机制问题（v8c 修）：①NAK 重试被 5s 超时误杀；②XFRC 不置位（ACK 判据替代）；③ep0 mask 方向残留（按 eptype+方向现算恒含 CHH）；chan_wait 改 500ms 切片轮询 progress。**v8d 补上能力层真根因：DWC2 RX FIFO 太小**——slave 模式核心发 IN token 前要求 RX FIFO 容纳"通道 max packet+状态字"，HS bulk mps=512B→128 words+~3 状态字≈131 words > 旧 GRXFSIZ 默认 128 → 核心静默拒绝发 token（CHENA=1、HCINT=0、零中断）；mps=8/64 控制传输只需 ~19 words 所以枚举全过——完美解释"枚举过、bulk 死"。修复：Kconfig 三 FIFO 默认 128/96/96→**512/256/224**（=SDK hal_usb_core.c GRXFSIZ=0x200/NPTX 0x100/PTX 0xE0）。待板上复测：插 U 盘应枚举出 sda 并可 mount vfat、ls 出文件 |
