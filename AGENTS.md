...
| `hd_rk3506_rpmsgtest.c` 阻塞读卡死                                        | ⚠️**A7 侧全修, M0 未消费待查 (v8b 探针实测)** | 主循环阻塞 read() 且超时判定在 read 之后 → M0 不回包时 read 永久阻塞。v8 改 poll(剩余 ms)+read；v8b 修复 rpmsg 共享窗 cached→uncached（M0 无 cache 无一致性，vring 元数据失同步根因）。**v8b 板上探针实测**：`A7->M0 (avail=1 used=0) M0->A7 (avail=64 used=0)`——ping 已入 vring1、A7 种子 64 可见（**uncached 修复生效**），但 M0 **从未消费**且从未发送任何报文 = M0 未启动或卡 link-up 前。**v8c 重大发现**：全项目驱动日志宏误用 INPUT 子系统 ierr/iwarn/iinfo（全部静默），"M0 firmware booted"（rptun _info）从未真正打过——M0 启动路径实际状态第一次可见待 v8c 复测：若 boot 日志仍无 "M0 firmware booted" 或出现 SIP_MCU_CFG/SMC 报错，按日志修启动链；若 booted 但仍不消费，查 mailbox3 kick 到达 M0 侧。端点 rpmsg-mcu0-echo（路由靠 dst 0x4003）。A7/M0 协议链路静态核对一致：link_id 0x03、ept 0x4003、A7 set_status→notify→mailbox3 kick、A7 收包 isr RPTUN_NOTIFY_ALL→两 vring 全处理 |
| U 盘 MSC bulk 传输挂死                                                    | ⚠️**根因修复 (v8c)，待板上验收** | v8b INFO 跟踪定位三层问题：①设备上电后对首请求长 NAK，驱动 NAK 重试循环在 ISR 里静默跑，5s 超时误把"正在重试"当"停滞"强拆（每个 ctrl IN 首试白等 5s）；②NAK 重试序列后核心偶发不置 XFRC——数据已收齐（xfrd==buflen）但 HCINT 只剩未 mask 的 ACK，通道空转；HCINTMSK 基线无 CHH，自 halt 完全不可见（偏离 u-boot dwc2 恒使能 CHHLTD）；③ep0 通道 IN/OUT 交替，mask 在 configure 时按当时方向写死，反方向用错。v8c：`rk3506_chan_intmsk()` 按 eptype+方向现算并恒含 CHH；transfer_start 每笔清 raw+重写 mask；ACK 判据（xfrd==buflen 或短包→按 XFRC 完成传输）；NYET→EAGAIN（HS bulk OUT 流控）；chan_wait 改 500ms 切片轮询 progress——有中断进展续期（3s 窗口/30s 硬上限），真停滞才转储+取消（转储已加 GINTMSK+progress）。待板上复测：插 U 盘应枚举出 sda 并可 mount vfat |
