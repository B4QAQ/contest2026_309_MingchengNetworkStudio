...
| `hd_rk3506_rpmsgtest.c` 阻塞读卡死                                        | ✅**已修复 (v8b 含根因)** | 主循环阻塞 read() 且超时判定在 read 之后 → M0 不回包时 read 永久阻塞。v8 改 poll(剩余 ms)+read；**v8b 定位真正根因并修复**：rk3506_start.c 的 MMU 身份映射把 rpmsg 共享窗 0x03c00000-0x03e00000 也映成 cached，而 M0 无 cache 无一致性 → A7 的 vring 元数据停在 dcache、M0 读 DDR 陈旧值，数据面完全失同步（mailbox kick 走寄存器不受影响，所以 M0 可能已 link-up）。修复：共享窗 2MiB 改映射 device/uncached（60+2+66 段）。rpmsgtest 超时时自动打印 vring 探针（A7→M0 avail/used = ping 是否入队/被消费；M0→A7 avail/used = M0 是否活着并公告过）定位断点。端点改名 rpmsg-mcu0-test→**rpmsg-mcu0-echo**（避开 M0 NS announce 同名节点冲突；路由靠 dst 0x4003）。A7/M0 协议链路静态核对一致：link_id 0x03、ept 0x4003、A7 set_status→notify→mailbox3 kick、A7 收包 isr RPTUN_NOTIFY_ALL→rproc_virtio_notified(ANY) 两 vring 全处理。M0 实际行为待 v8b 板上验证 |
| U 盘 MSC bulk IN 挂死                                                     | ⚠️**诊断版 v8b** | 现象：枚举成功（控制传输全过）、TestUnitReady 的 CSW bulk IN 无响应挂死（CBW OUT 已被 ACK），/dev/sda 不出现，拔线才报 -32（拔线中止非 STALL）。静态分析：驱动 ctrl/bulk 路径机制相同（ctrl 枚举已验证），DWC2 PID 编码正确（DATA1=2）、FIFO/通道配置与参考一致 → 疑设备侧沉默（CBW 内容/边缘时序）。v8b 已加：①通道等待 5s 超时 + 寄存器转储（GINTSTS/HAINT/HCINT/HCINTMSK/HCCHAR/HCTSIZ，GRXSTSP 有意不读防弹栈）；②transfer_start/rxflvl/hc(done) iinfo 跟踪；③defconfig 开 DEBUG_USB_WARN/INFO（诊断后可回收）。待板上复测取日志定位 |
