...
| `hd_rk3506_rpmsgtest.c` 阻塞读卡死                                        | ⚠️**v8e 找到真根因并修复, 待板上验收** | 主循环阻塞 read() 且超时判定在 read 之后 → M0 不回包时 read 永久阻塞。v8 改 poll(剩余 ms)+read；v8b 修复 rpmsg 共享窗 cached→uncached（M0 无 cache 无一致性，vring 元数据失同步根因）。**v8e 定位真根因**：`rk3506_mbox_init()` 原"开 PCLK_MAILBOX"写的是 `WE|bit13`（值含 1），Rockchip 门控 SET_TO_DISABLE 语义（内核 GFLAGS=HIWORD_MASK\|SET_TO_DISABLE；本仓 SPI1 用 gates<<16 即数据位=0 才开门）→ 写 1 实为**关死 mailbox pclk**：v8b 起每次 boot A7 亲手关掉 mailbox 时钟，探针全零、kick 全丢、M0 永远收不到，mbox_send 忙等读 0 静默"成功"。修复：写 0 带 WE 开 PCLK_MAILBOX+PCLK_INTMUX(CON6 b13,14)，mcu_boot 补开 PCLK_UART4+SCLK_UART4(CON11 b8,13)。**v8e 探针 v2**：INTEN 可能只写不可读，判读改以 CRU 门控转储(CON5/6/11)+MBOX3 A2B_CMD/DATA 回读为准（CMD=0x03/DATA=0x524d5347= kick 落地；CON6 bit13=1=pclk 仍被门控）。端点 rpmsg-mcu0-echo（路由靠 dst 0x4003） |
