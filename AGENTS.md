...
| curl HTTPS 传完不退出 (Ctrl+C 才"出结果")                                  | ❌**v8c 记录作废, 机理错误, 未修复** | **v8j 自我更正**: 原记录称"NuttX socket 阻塞且 curl 未设 O_NONBLOCK 导致 `mbedtls_close()` 的无条件 `mbedtls_ssl_read()` 永久阻塞"——**该机理站不住**: ① curl 在 `lib/cf-socket.c:1079` 无条件调 `curlx_nonblock(ctx->sock, TRUE)`; ② `curl_config.h:194` 定义 `HAVE_FCNTL_O_NONBLOCK`, `curl_setup_once.h:220` 的 `sfcntl`=`fcntl`(非 lwip); ③ NuttX 的 `F_SETFL` 经 `fs_fcntl.c:99-113` 转 `file_ioctl(FIONBIO)`, `netdev_ioctl.c:1756` 置 `conn->s_flags |= _SF_NONBLOCK`, `tcp_recvfrom.c:721` 认它。**socket 本来就是非阻塞的**, 那个读应立即返回 WANT_READ。且该补丁**已不存在**——用户要求"curl 使用仓库原始版本", `external/curl/curl` 回到 `8c2a01f3e`, `mbedtls.c:1050` 的无条件读原样还在。另排除: 本仓 curl 8.4.0-DEV, `Curl_conn_shutdown` 优雅关闭超时是 8.8+ 才有的, 与此无关。**下一步先取证不改码**(见 `MORNING_CHECKLIST.md` §14): `curl -o /tmp/o URL; echo DONE=$?` 区分"真挂死"与"stdout 全缓冲未刷"; `curl -v` 看停在哪个阶段; `curl --max-time 20` 看能否被打断。**否决记录**: 另一 AI 建议的 (a) `CURLOPT_FORBID_REUSE`+`HTTP_VERSION_1_0` 机制上能绕开(服务器主动关 → close_notify 或 FIN 都让读返回)但不治本、全局牺牲 keep-alive/HTTP1.1, 且 NSH 的 curl 自建 easy handle, "在初始化代码里设置"等于改 curl 源码(与原始版本要求冲突); (b) "改 `nuttx/drivers/net/tcp_*.c` 让 FIN 映射 POLLHUP" **三重错误**: 该路径不存在(TCP socket poll 在 `nuttx/net/tcp/tcp_netpoll.c`)、NuttX 早已在 4 处设 `POLLERR|POLLHUP`、且卡点根本没走 poll() 而我们的场景服务器压根没发 FIN |
...
| NSH 命令行长度 | ✅**已放宽 (v8j)** | 用户反馈"单条命令有最长字符限制"。根因是配置不是缺陷：defconfig 从未写过这两项，一直吃默认值 `CONFIG_NSH_LINELEN=80`(`nsh.h:383`) 和 `CONFIG_NSH_MAXARGUMENTS=7`。curl/ota 的 https URL 轻松超 80 列。已改 **256 / 16**（用户拍板）。代价有界：`cn_line[]` 在每 session 的 `console_stdio_s`(堆)，`nsh_parse.c:2575` 一个 `tracebuf[LINELEN+1]` 在栈上；nsh_main 栈 8088 B 实测仅用 328 B |
| `rk3506_iomux.c` 日志 | ✅**已删 (v8k)** | 用户要求去掉。该文件 4 条 `_info()`（`IOC mux: reg=...` / `SPI1/FSPI/I2C0 IOMUX:`），无错误路径日志可保留，全删；连带删掉只被日志用到的 `inttypes.h`/`debug.h`/`syslog.h`。顺带修掉既有 `RK3506_GRF_PMU_ADDR` 重复定义告警（本地定义与 `hardware/rk3506_memorymap.h:99` 同值重复，删本地版） |
| `rk3506_fspi.c` 日志 | ✅**已删 (v8l)** | 用户点名删 `hw init done` 和 `dll: reference id`。该文件 8 条 `_info`（VER/hw init done/dll reference/cells 逐条/training OK/failed/READ_ID/READ_ID failed）、无 `_err`/`_warn` 可保留，全删；连带 `inttypes.h`/`debug.h`/`syslog.h`。DLL 调谐控制流一行未动。`rk3506_sfc_irq_unmask` 的 unused-function 告警为既有问题（注释注明 verbatim kept for symmetry），去留未定。注意：`rk3506_spinand_fspi.c:894` 还有一条 `syslog(LOG_INFO, "FSPI: SPI NAND READ_ID ...")`（MTD 层的同名打印，本次未动） |
